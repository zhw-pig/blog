import type { Recordable } from '@zhw-core/typings';
import type {
  FormState,
  GenericObject,
  ResetFormOpts,
...
import type { FormActions, FormSchema, ZhwFormProps } from './types';

import { toRaw } from 'vue';

import { Store } from '@zhw-core/shared/store';
import {
  bindMethods,
  createMerge,
  isDate,
  ...
  StateHandler,
} from '@zhw-core/shared/utils';

function getDefaultState(): ZhwFormProps {
  return {
    actionWrapperClass: '',
    collapsed: false,
    collapsedRows: 1,
...
export class FormApi {
  ...
  private prevState: null | ZhwFormProps = null;

  // private api: Pick<ZhwFormProps, 'handleReset' | 'handleSubmit'>;
  public form = {} as FormActions;
  isMounted = false;

  public state: null | ZhwFormProps = null;

  stateHandler: StateHandler;

  public store: Store<ZhwFormProps>;

  constructor(options: ZhwFormProps = {}) {
    const { ...storeState } = options;

    const defaultState = getDefaultState();

    this.store = new Store<ZhwFormProps>(
      {
        ...defaultState,
        ...storeState,
      },
  ...
  setState(
    stateOrFn:
      | ((prev: ZhwFormProps) => Partial<ZhwFormProps>)
      | Partial<ZhwFormProps>,
  ) {
