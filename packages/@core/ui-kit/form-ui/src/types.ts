import type { ZhwButtonProps } from '@zhw-core/shadcn-ui';
import type { ClassType } from '@zhw-core/typings';
...
export interface ActionButtonOptions extends ZhwButtonProps {
  [key: string]: any;
  content?: string;
  show?: boolean;
}

export interface ZhwFormProps<
  T extends BaseFormComponentType = BaseFormComponentType,
> extends Omit<
    FormRenderProps<T>,
...
export type ExtendedFormApi = {
  useStore: <T = NoInfer<ZhwFormProps>>(
    selector?: (state: NoInfer<ZhwFormProps>) => T,
  ) => Readonly<Ref<T>>;
} & FormApi;
